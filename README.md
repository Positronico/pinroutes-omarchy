...
3. **Install helper** (required for applying routes) — click **Install helper** in the panel's HELPER section to copy the install command, then paste and run it once in a terminal: `sudo /bin/bash -c "$(cat '<plugin-dir>/helper/pinroutes-helper-install')" pinroutes-approve --approve <your routes>` (your shell reads the script; root only ever sees its text). It installs the helper and approves your current routes; after that, approved routes re-apply silently, which is what makes background auto-reapply possible. When you later add or edit a route, the panel offers **Approve route changes** (one pkexec prompt against the installed approver) to extend silent re-apply to it.
...
- **The plugin never passes executable content across the privilege boundary, and root never opens a user-writable code path.** Installation is a deliberate manual step: the panel's "Install helper" action only *copies* the exact command to your clipboard — `sudo /bin/bash -c "$(cat '<plugin-dir>/helper/pinroutes-helper-install')" pinroutes-approve --approve <your route pairs>` — and **you** run it in your own terminal, after reviewing the script if you wish. The `$(cat …)` runs in *your* shell before any privilege exists, so the content root executes is bound at that moment; the installer refuses direct execution of any non-root-owned file, its deadline re-exec travels through memory (`$BASH_EXECUTION_STRING`), and the persisted approver is written from that same in-memory text — the checkout is never reopened by root. From then on, the only privileged executables the plugin ever invokes are the two **fixed root-owned components** that install created: `pinroutes-helper` via `sudo -n` and `pinroutes-approve` via `pkexec` — always with validated route pairs as the only data crossing the boundary.
- **The installer is a real transaction: staged, validated, journaled, published atomically, rolled back on failure — and crash-recoverable.** Every pre-existing target must be a root-owned regular file carrying the `managed-by: pinroutes` marker (anything else aborts — nothing unknown is ever clobbered). All four artifacts are staged into their target directories and validated (`bash -n`, `visudo -cf`); every existing target is backed up and a durable journal (fsynced before the first live rename) records each target with its backup and **old/new content digests**; on an update the **old sudoers grant is revoked first**, then files are published by atomic renames with the new grant **last**; the commit point is an atomic **rename of the journal to a committed marker** (never an ambiguous unlink), made durable before any recovery data is released. Any in-process failure rolls back from the backups; an interruption that kills the process outright (SIGKILL, power loss) is recovered by the next run from the journal *before* anything is swept. Recovery and rollback are **fail-closed and digest-verified**: recovery never assumes anything from a file's absence — it proves every target against the journaled digests and completes the transaction only when *all* targets are provably new (finish the commit) or *all* can be made provably old (verified restores); any mixed or unprovable state — including partially persisted backup deletions after a crash — retains the journal and every surviving backup and refuses further transactions. A failed restore never deletes the live target; pre- and post-publication `sync` failures abort (with journaled rollback); a commit-durability failure retains the marker and all backups for next-run verification instead of releasing anything. Rollback and recovery both honor the **grant-absent-first invariant**: any live sudoers grant is removed and its absence proven with an **unconditional sudoers-directory durability barrier — even when the path is already absent** (a prior removal may not yet be durable) — before other targets are touched, mutation **halts on the first failed revoke or restore** (retaining the journal and every backup), and the old grant is restored from its backup (digest-verified in recovery) only *last*, after every other restore has succeeded durably — an interruption leaves the grant absent, never active over a mixed component set. Rollback obligations are recorded **before** each live rename or removal, so an asynchronous signal landing between the two finds either a no-op restore or an already-tracked mutation — never an untracked one — and the journal is never released until every target is byte-exactly old or committed new — proven by deterministic signal-injection tests at every publication boundary in CI (before/after each rename including the commit marker, and before/after the revoke), with durability barriers asserted structurally at each site. The journal is accepted only as a **complete transaction**: exactly one canonical, digest-well-formed entry per managed target; any missing, duplicate, extra, malformed, or legacy line — or a journal/directory failing ownership validation — is retained untouched for inspection and blocks further transactions rather than being partially obeyed or discarded. The sudoers subject is the validated **numeric `#uid`** form, so adversarial account names (a user literally named `ALL`) cannot change the rule's meaning. Uninstall recovers any interrupted transaction first, removes only marker-verified files, and treats a failed revocation barrier as fatal — no other managed file is touched until the grant's absence is durable. It derives the invoking user from **`PKEXEC_UID`/`SUDO_UID`** (never argv or ambient env), re-validates and caps route pairs (100), and installs:
  ...
  - `/etc/sudoers.d/pinroutes` (root:root 0440): a pinned `secure_path` and an `env_delete` for interpreter-startup variables (`BASH_ENV`, `ENV`, `SHELLOPTS`, `BASHOPTS`, `PS4`, the deadline guard) for the helper, plus `#<your-uid> ALL=(root) NOSETENV: NOPASSWD: /usr/local/bin/pinroutes-helper` (numeric `#uid` User_List form; `NOSETENV` pins the rule against ambient `SETENV` site policy)
...
- **Bounds and supervision everywhere**: route count (100) and name length (100) enforced at creation (`addRoute` refuses past the limit), update, *and* restore, so the model, config payload, and privileged argv can never exceed what the root side accepts; every operation — `replace` *and* `delete` — must match a full approved `<network> <gateway>` pair; **both privileged components enforce their own whole-invocation deadline root-side** (each re-execs once under `timeout -k 5 60`, TERM→KILL, in the privileged context — the unprivileged caller cannot disarm it under default sudo/pkexec environment policy), with a per-`ip`-call `timeout` and the 128-op cap inside that envelope; route-table output capped at the producer (`ip | head -c` with `pipefail`); every short-lived unprivileged process wrapped in kernel-enforced `timeout -k` with QML watchdog timers as an additional backstop; the long-lived netlink monitor restarts with exponential backoff. No network access, no downloads, no bundled binaries, no services. Residual, by design: a same-user process can re-`replace` or `delete` routes *within the approved set* — that set is exactly what the user authorized.
