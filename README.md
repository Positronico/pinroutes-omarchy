...
- **The installer is a real transaction: staged, validated, published atomically, rolled back on failure.** Every pre-existing target must be a root-owned regular file carrying the `managed-by: pinroutes` marker (anything else aborts — nothing unknown is ever clobbered). All four artifacts are staged into their target directories, validated (`bash -n`, `visudo -cf`), then published by atomic renames with the sudoers privilege grant **last**; pre-existing managed files are backed up first, and any failure mid-publication restores every already-replaced target from its backup (in reverse order), so no failure mode leaves a privilege grant over a mixed installation. Uninstall removes only marker-verified files. It derives the invoking user from **`PKEXEC_UID`/`SUDO_UID`** (never argv or ambient env), re-validates and caps route pairs (100), and installs:
